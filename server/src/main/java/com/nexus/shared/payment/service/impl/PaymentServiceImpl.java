package com.nexus.shared.payment.service.impl;
...
import com.nexus.shared.payment.gateway.dto.*;
...
import com.nexus.shared.payment.repository.PaymentMethodRepository;
import com.nexus.shared.payment.repository.PaymentRefundRepository;
import com.nexus.shared.payment.repository.PaymentRepository;
import com.nexus.shared.payment.service.PaymentService;
...
public class PaymentServiceImpl implements PaymentService {
    ...
    @Override
    @Transactional
    public PaymentResponse processPayment(UUID userId, UUID domainId, UUID tenantId, ProcessPaymentRequest request) {
        log.info("Processing payment: userId={}, domain={}, amount={}", userId, domainId, request.amount());
        ...
        if (request.paymentMethodId() != null) {
            PaymentMethod paymentMethod = paymentMethodRepository
                .findByUserIdAndDomainIdAndTenantIdAndId(userId, domainId, tenantId, request.paymentMethodId())
        ...
        com.nexus.shared.payment.gateway.dto.PaymentResponse gatewayResponse;
        ...
        if (request.paymentGatewayTransactionId() != null && !request.paymentGatewayTransactionId().isEmpty()) {
            ...
            if (existingPayment.isPresent()) {
                ...
                // Verify it belongs to the same user, domain, and tenant
                if (payment.getUserId().equals(userId) && payment.getDomainId().equals(domainId) && payment.getTenantId().equals(tenantId)) {
                    ...
                    log.warn("Payment with transaction ID {} exists but belongs to different user/domain/tenant. userId={}, domainId={}, tenantId={}, existingUserId={}, existingDomainId={}, existingTenantId={}", 
                        request.paymentGatewayTransactionId(), userId, domainId, tenantId, payment.getUserId(), payment.getDomainId(), payment.getTenantId());
    ...
    @Override
    @Transactional
    public PaymentResponse refundPayment(UUID userId, UUID domainId, UUID tenantId, List<String> userRoles, RefundPaymentRequest request) {
        ...
        // Verify domain and tenant
        if (!payment.getDomainId().equals(domainId) || !payment.getTenantId().equals(tenantId)) {
            throw new BusinessException(
                ErrorCode.ACCESS_DENIED,
                "Payment belongs to different domain or tenant"
            );
        }
    ...
    @Override
    @Transactional
    public PaymentMethodResponse savePaymentMethod(UUID userId, UUID domainId, UUID tenantId, SavePaymentMethodRequest request) {
        log.info("Saving payment method: userId={}, domain={}, type={}", userId, domainId, request.type());
        ...
        if (Boolean.TRUE.equals(request.isDefault())) {
            List<PaymentMethod> existingMethods = paymentMethodRepository.findByUserIdAndDomainIdAndTenantId(userId, domainId, tenantId);
    ...
    public List<PaymentMethodResponse> getPaymentMethods(UUID userId, UUID domainId, UUID tenantId) {
        log.debug("Getting payment methods: userId={}, domain={}", userId, domainId);
        
        List<PaymentMethod> methods = paymentMethodRepository.findByUserIdAndDomainIdAndTenantId(userId, domainId, tenantId);
    ...
    @Override
    @Transactional
    public void deletePaymentMethod(UUID userId, UUID domainId, UUID tenantId, UUID paymentMethodId) {
        log.info("Deleting payment method: paymentMethodId={}", paymentMethodId);
        
        PaymentMethod paymentMethod = paymentMethodRepository
            .findByUserIdAndDomainIdAndTenantIdAndId(userId, domainId, tenantId, paymentMethodId)
            .orElseThrow(() -> new BusinessException(
                ErrorCode.RESOURCE_NOT_FOUND,
                "Payment method not found"
            ));
    ...
    @Override
    @Transactional(readOnly = true)
    public Page<PaymentResponse> getPaymentHistory(UUID userId, UUID domainId, UUID tenantId, Pageable pageable) {
        log.debug("Getting payment history: userId={}, domain={}", userId, domainId);
        
        Page<Payment> payments = paymentRepository.findByUserIdAndDomainIdAndTenantId(userId, domainId, tenantId, pageable);
        return payments.map(this::toResponse);
    }
    
    @Override
    @Transactional(readOnly = true)
    public PaymentResponse getPaymentStatus(UUID userId, UUID domainId, UUID tenantId, List<String> userRoles, UUID paymentId) {
        ...
        if (payment.getStatus() == Payment.PaymentStatus.PENDING && payment.getGatewayTransactionId() != null) {
            try {
                com.nexus.shared.payment.gateway.dto.PaymentResponse gatewayResponse = paymentGateway.getPaymentStatus(payment.getGatewayTransactionId());
    ...
    @Override
    @Transactional
    public com.nexus.shared.payment.model.response.CreateOrderResponse createOrder(UUID userId, UUID domainId, UUID tenantId, com.nexus.shared.payment.model.request.CreateOrderRequest request) {
        log.info("Creating Razorpay order for client-side checkout: userId={}, domain={}, amount={}", userId, domainId, request.amount());
        ...
        return new com.nexus.shared.payment.model.response.CreateOrderResponse(
            request.orderId(),
            razorpayOrderId,
            request.amount(),
            request.currency() != null ? request.currency() : "INR",
